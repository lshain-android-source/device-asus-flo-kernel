...
static ssize_t freg_proc_write( struct file* filp, const char __user *buf, unsigned long len, void* data ){
	...
	if( copy_from_user( page, buf, len ) ){
		printk( KERN_ALERT"Faild to copy buff from user.\n");
		err = -EFAULT;
		goto out;
	}
...
static int __init freg_init( void ){
	...
	if( err < 0 ){
		printk( KERN_ALERT"Faild to alloc char dev region.\n");
		goto fail;
	}
	...
	if( !freg_dev ){
		err = -EFAULT;
		printk( KERN_ALERT"Faild to alloc freg device.\n");
		goto unregister;
	}

	err = __freg_setup_dev( freg_dev );
	if( err ){
		printk( KERN_ALERT"Faild to setup freg device: %d.\n", err);
		goto cleanup;
	}
	...
	if( IS_ERR(temp) ){
		err = PTR_ERR( temp );
		printk( KERN_ALERT"Faild to create freg device.\n");
		goto destroy_class;
	}
...
static void __exit freg_exit( void ){
	...
	printk( KERN_ALERT"Destory freg deice." );
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Fake Regiter Driver");
